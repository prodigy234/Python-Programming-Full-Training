...
st.title("🗳️ 2023 Nigerian Presidential Election Analysis (Advanced Dashboard)")
...
st.plotly_chart(px.bar(total_df_sorted, x="Candidate", y="Total Votes", title="Total Votes per Candidate", color="Candidate"))
st.dataframe(total_df_sorted, use_container_width=True)
st.download_button("📥 Download Vote Summary", total_df.to_csv(index=False), "votes_summary.csv")
...
st.plotly_chart(px.pie(percent_df_sorted, names="Candidate", values="Vote %", title="Vote Share Distribution"))
...
st.plotly_chart(px.bar(region_df, barmode="stack", title="Votes per Candidate by Region"))
...
# -----------------------------
# Interactive Filters (Bonus)
# -----------------------------
with st.sidebar:
    selected_region = st.selectbox("Select Region to Filter", list(regions.keys()))
    filtered_states = regions[selected_region]
    st.write("Filtered States:", filtered_states)
    st.dataframe(df[df["State"].isin(filtered_states)])
